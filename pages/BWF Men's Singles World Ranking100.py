...
if selected_date2:
    ...
    with row4_1:
        ...
        # 假设 gdf 中的数值栏位名为 'value'
        value_column = 'player_count'
        
        # 创建数值正则化范围
        norm = Normalize(vmin=gdf1[value_column].min(), vmax=gdf1[value_column].max())
        
        # 定义样式函数（固定蓝色，透明度根据数值设置）
        def style_function(feature):
            value = feature["properties"][value_column]
            opacity = norm(value)  # 将数值正则化到 [0, 1] 范围
            return {
                "fillColor": "#0000FF",  # 固定蓝色 (十六进制格式)
                "color": "black",        # 边框颜色
                "weight": 1,             # 边框宽度
                "fillOpacity": opacity,  # 根据数值调整透明度
            }
        
        # 创建地图并添加 GeoDataFrame
        m = leafmap.Map(center=(0, 0), zoom=2)
    ...
    with row4_2:
        ...
        # 定义样式函数（固定红色，透明度根据数值设置）
        def style_function(feature):
            value = feature["properties"][value_column]
            opacity = norm(value)  # 将数值正则化到 [0, 1] 范围
            return {
                "fillColor": "#ff0000",  # 固定红色 (十六进制格式)
                "color": "black",        # 边框颜色
                "weight": 1,             # 边框宽度
                "fillOpacity": opacity,  # 根据数值调整透明度
            }
