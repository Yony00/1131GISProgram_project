...
from scrape_bwf_ranking_by_date import WDscrape_bwf_ranking_by_date as scrape_bwf_ranking_by_date  # 引入第二次爬蟲的函數
import numpy as np
...
# 設定頁面配置為寬屏模式
st.set_page_config(page_title="Women's Doubles", layout="wide", page_icon=":🏸")
st.markdown(
    """
    <style>
    .stApp {
...
logo = "https://bwfworldtourfinals.bwfbadminton.com/wp-content/uploads/2021/12/20211203_1044_WorldTourFinals2021_BPRS2902-1-2048x1393.jpg"
...
with row0_1:
    st.write(
        ...
        1.選手排名資料表格\n
        2.可供選擇的日期欄位\n
        3.各國在前一百名中佔有的選手(組)數量 長條圖\n
        4.各國在前一百名中佔有的選手(組)數量 分層設色地圖\n
        注：可點選多邊形查看國名、選手(組)數、選手名\n
        ##
        """
    )
with row0_2:
...
if "df_initial" not in st.session_state:  # 只有在第一次爬蟲未完成時才會執行
    try:
        url = "https://bwf.tournamentsoftware.com/ranking/category.aspx?id=43595&category=475&C475FOC=&p=1&ps=100"
...
if selected_date1:
    try:
        ...
        # 顯示選擇日期的排名資料於 row1_1
        with row1_1:
            st.write(f"下表為 {selected_date1}  時 女子雙打排名資料")
...
if selected_date2:
    try:
        ...
        # 顯示選擇日期的排名資料於 row1_2
        with row1_2:
            st.write(f"下表為 {selected_date2}  時 女子雙打排名資料")
...
if not selected_date2:
    ...
    m.add_gdf(
        gdf1,
        layer_name=f"BWF Women's Doubles World Ranking for {selected_date1}:",
...
if selected_date2 and user_choice == "是":
 ...
 # 添加 gdf1 到地圖
    folium.GeoJson(
        gdf1,
        name=f"BWF Women's Doubles World Ranking for {selected_date1}",
    ...
    if output1 :
        ...
        folium.GeoJson(
            gdf2,
            name=f"BWF Women's Doubles World Ranking for {selected_date2}",
...
if user_choice == "否" and selected_date2 :
    ...
    m2.add_gdf(
        gdf2,
        layer_name=f"BWF Women's Doubles World Ranking for {selected_date2}:",
        style_function=style_function,
        info_mode='on_click'
    )
    # 顯示地圖
